...
async function cekStatusPesanan(kategori) {
    ...
    try {
        ...
        let data3 = (res3.data || []).map(o => ({
            ...
            product_name: `[PPOB] ${o.sku_code} (${o.customer_no})`,
            id: o.ref_id
        }));
    ...
    '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-brand-info underline font-bold hover:text-white transition-colors">$1</a>'
            ...
            if (namaProduk.includes('[DEPOSIT]')) {
                ...
                const autoRefreshSaldo = setInterval(() => {
                    ...
                    checkCount++;
                    if (checkCount >= 5) clearInterval(autoRefreshSaldo);
                }, 2000);
            }
            if (namaProduk.includes('[VIP]')) {
        ...
        if (data && data.length > 0) {
            container.innerHTML = data.map(item => {
                ...
                if (item.fee_ditanggung_pembeli) {
                    ...
                    hargaTampilEtalase = hargaAsli + hitungPotonganSeller(hargaAsli);
                } else {
...
function panduanCekLegalitas(tipe) {
    ...
    if (tipe === 'NIB') {
        ...
        actionHtml = `
            <a href="https://ui-login.oss.go.id/verify/UBsCIAZhWmRfagRiAmBSNgJmCTcDMAtoAmFRYABjBGFTZlZtBWleYVZlBWVVMg==" target="_blank" rel="noopener noreferrer" onclick="document.getElementById('alert-ok').click()" class="w-full bg-brand-info text-brand-dark py-3.5 rounded-xl font-extrabold active:scale-95 transition-all text-xs uppercase tracking-wider shadow-[0_4px_15px_rgba(70,179,255,0.4)] flex justify-center items-center gap-2 mb-3">
                Lihat Sertifikat OSS 1 <i class="fas fa-external-link-alt"></i>
            </a>
            <a href="https://ui-login.oss.go.id/verify/XhUBIwFmW2VVYFYwUzFUMANkCD8GN1E0UDRWZlMyDmVQZlVrA2QNOwA1AWcFbg==" target="_blank" rel="noopener noreferrer" onclick="document.getElementById('alert-ok').click()" class="w-full bg-brand-info text-brand-dark py-3.5 rounded-xl font-extrabold active:scale-95 transition-all text-xs uppercase tracking-wider shadow-[0_4px_15px_rgba(70,179,255,0.4)] flex justify-center items-center gap-2 mb-2">
                Lihat Sertifikat OSS 2 <i class="fas fa-external-link-alt"></i>
            </a>
        `;
    } else if (tipe === 'AHU') {
