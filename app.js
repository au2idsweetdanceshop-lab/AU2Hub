...
async function loadVideos() {
    ...
    if (allVideosData.length === 0) {
        if (fakeLoader) {
            ...
            if (fakeProgress) {
                ...
                setTimeout(() => fakeProgress.style.width = '50%', 500);
                setTimeout(() => fakeProgress.style.width = '90%', 1500);
            }
        }
    }
    ...
    try {
        ...
        container.innerHTML = '';
        const reversedVideos = [...allVideosData].reverse();
        
        if (reversedVideos.length === 0) {
            container.innerHTML = '<div class="w-full h-full flex items-center justify-center text-xs text-gray-500">Belum ada video di feed.</div>';
        } else {
            const htmlString = reversedVideos.map((vid) => `
                <div class="snap-start w-full h-full flex-shrink-0 relative flex items-center justify-center overflow-hidden bg-black/95 px-0 sm:px-4 py-0 sm:py-6">
                    <div class="w-full max-w-sm aspect-[9/16] relative bg-brand-dark mx-auto h-full sm:h-auto sm:rounded-3xl overflow-hidden shadow-2xl">
                        <div class="absolute inset-0 flex items-center justify-center z-0"><div class="w-12 h-12 border-4 border-brand-accent/20 border-t-brand-accent rounded-full animate-spin"></div></div>
                        ...
                        <div class="absolute bottom-0 left-0 w-full h-3 z-50 cursor-pointer group touch-none flex flex-col justify-end pb-1"
                             onpointerdown="startSeek(event, this)" onpointermove="doSeek(event, this)" onpointerup="endSeek(event, this)" onpointercancel="endSeek(event, this)">
                            <div class="w-full h-1 bg-white/30 relative">
                                <div class="progress-fill h-full bg-white w-0 relative pointer-events-none transition-all duration-75 ease-linear"></div>
                            </div>
                        </div>
                        ...
                        <div class="absolute bottom-0 left-0 w-full p-6 flex justify-between items-end z-20 pointer-events-none bg-gradient-to-t from-black/80 to-transparent pb-10 sm:pb-6">
                            <div class="text-white drop-shadow-lg w-[75%] pointer-events-auto">
                                <div class="flex items-center gap-2 mb-2">
                                    <img src="${vid.avatar_url || 'https://ui-avatars.com/api/?name=User'}" class="w-8 h-8 rounded-full border border-white/50 object-cover cursor-pointer" onclick="viewUserProfile('${vid.user_id}')">
                                    <p class="font-bold text-[15px] shadow-black drop-shadow-md cursor-pointer hover:underline" onclick="viewUserProfile('${vid.user_id}')">@${vid.nickname || "Player"}</p>
                                    ${(!currentUser || currentUser.id !== vid.user_id) ? `<button onclick="feedToggleFollow('${vid.user_id}', this)" class="bg-brand-accent/20 text-brand-accent text-[9px] font-bold px-2 py-0.5 rounded-full border border-brand-accent/30"><i class="fas fa-plus text-[10px]"></i></button>` : ''}
                                </div>
                                <div onclick="this.classList.toggle('expanded')" class="caption-text text-[13px] opacity-90 shadow-black drop-shadow-md cursor-pointer pr-2 leading-relaxed">${formatCaption(vid.caption)}</div>
                            </div>
            ...
            const videoCards = container.querySelectorAll('.snap-start');
        ...
        container.innerHTML = '<div class="w-full h-full flex items-center justify-center text-xs text-red-500">Gagal memuat feed video.</div>';
    } finally {
        if (fakeLoader) {
            if (fakeProgress) fakeProgress.style.width = '100%';
            setTimeout(() => {
                fakeLoader.classList.add('opacity-0');
                setTimeout(() => {
                    fakeLoader.classList.add('hidden');
                    fakeLoader.classList.remove('opacity-0');
                }, 500);
            }, 500);
        }
    }
...
function toggleGlobalAudio() {
    ...
    showToast(isGlobalMuted ? "Suara Video Dimatikan" : "Suara Video Dinyalakan", "info");
}
