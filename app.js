...
// ==========================================
// 2. FUNGSI UTILITIES (UI & ALAT BANTU)
// ==========================================
...
function customPrompt(title, defaultValue = '') {
    return new Promise((resolve) => {
        ...
        modal.classList.remove('hidden'); modal.classList.add('flex'); inputEl.focus();

        const cleanup = () => { modal.classList.add('hidden'); modal.classList.remove('flex'); btnOk.onclick = null; btnCancel.onclick = null; };
...
function openLightbox(imgUrl) {
    ...
    img.src = imgUrl; modal.classList.remove('hidden'); modal.classList.add('flex');
    void modal.offsetWidth; modal.classList.remove('opacity-0'); img.classList.remove('scale-95'); img.classList.add('scale-100');
}

function closeLightbox() {
    const modal = document.getElementById('lightbox-modal'); const img = document.getElementById('lightbox-img');
    modal.classList.add('opacity-0'); img.classList.remove('scale-100'); img.classList.add('scale-95');
    setTimeout(() => { modal.classList.add('hidden'); modal.classList.remove('flex'); img.src = ''; }, 300);
}
...
window.addEventListener('beforeunload', function (e) {
    if (isUploading) { e.preventDefault(); e.returnValue = 'Upload sedang berlangsung. Yakin ingin meninggalkan halaman?'; }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Event Listener Share Video Link
    ...
    document.getElementById('chat-room-input')?.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') document.getElementById('btn-send-room').click();
    });
});
...
function switchTab(tabId, event = null, isPush = true) {
    ...
    if (tabId === 'profile' && event !== null && currentUser) {
        if (viewedUserId !== currentUser.id) { viewedUserId = currentUser.id; openUserProfile(currentUser.id); return; }
    }

    if (tabId !== 'profile' && tabId !== 'pembayaran' && tabId !== 'upload') { tabSebelumnya = tabId; } 
    if (tabId === 'layanan' && isPush && document.getElementById('pembayaran').classList.contains('active')) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    ...
    document.querySelectorAll('.nav-item').forEach(n => { n.classList.remove('active'); const icon = n.querySelector('i'); if (icon) icon.style.animation = 'none'; });

    targetSection.classList.add('active');
    if (tabId === 'sosial') loadVideos();
...
// ==========================================
// 4. MANAJEMEN SESI & AUTHENTIKASI
// ==========================================
async function checkSession() {
    document.getElementById('profile-loading').classList.remove('hidden'); document.getElementById('profile-loading').classList.add('flex');
    document.querySelectorAll('#profile-logged-in').forEach(el => el.classList.add('hidden')); document.getElementById('profile-logged-out').classList.add('hidden');

    const { data: { user } } = await supabaseClient.auth.getUser();
    
    document.getElementById('profile-loading').classList.add('hidden'); document.getElementById('profile-loading').classList.remove('flex');

    if (user) { 
        currentUser = user; await fetchProfile(); await fetchBlockedUsers(); checkGlobalUnreadMessages();
        ...
        updateUIForLoggedIn(); initPresence(); initGlobalMessageListener(); 
    } else { 
...
function openAuthModal() { history.pushState({ popup: 'auth' }, null, '#auth'); document.getElementById('modal-auth').classList.remove('hidden'); document.getElementById('modal-auth').classList.add('flex'); }
function closeAuthModal() { 
    const modal = document.getElementById('modal-auth'); modal.style.transition = 'opacity 0.3s ease'; modal.style.opacity = '0';
    setTimeout(() => { if (window.location.hash === '#auth') { history.back(); } else { modal.classList.add('hidden'); modal.classList.remove('flex'); } modal.style.opacity = '1'; }, 300);
}
...
async function handleAuth() {
    ...
    try {
        if (isAuthLogin) { 
            ...
            if (error) throw error; showToast("Login berhasil!", "success");
        } else { 
        ...
        document.getElementById('auth-email').value = ''; document.getElementById('auth-pass').value = ''; document.getElementById('auth-nick').value = '';
        if ("Notification" in window && Notification.permission !== "granted") Notification.requestPermission();
        await checkSession(); closeAuthModal();
    } catch (e) { showToast(e.message, "error"); } finally { btn.disabled = false; btn.innerText = isAuthLogin ? "Login" : "Daftar"; }
}

async function handleLogout() { 
    await supabaseClient.auth.signOut(); currentUser = null; userProfile = null; blockedUsersList = [];
    if (globalMessageSubscription) { supabaseClient.removeChannel(globalMessageSubscription); globalMessageSubscription = null; }
    checkSession(); showToast("Anda telah keluar.", "info");
}
...
function updateUIForLoggedIn() { 
    ...
    document.getElementById('header-user').innerHTML = `<div onclick="switchTab('profile')" class="flex items-center gap-2 cursor-pointer bg-white/5 pr-4 pl-1 py-1 rounded-full border border-white/10 active:scale-95 transition-transform"><img src="${ava}" class="w-7 h-7 rounded-full object-cover border border-brand-info"><span class="text-[10px] font-bold text-white">${userProfile?.nickname || 'User'}</span></div>`; 
    renderProfileVideos();
    if(currentUser) fetchFollowStats(currentUser.id);
}

function updateUIForLoggedOut() { document.getElementById('header-user').innerHTML = `<button onclick="openAuthModal()" class="text-[10px] font-bold bg-white/10 px-4 py-2 rounded-full border border-white/10 uppercase active:scale-95 transition-transform">Login / Daftar</button>`; }

// ==========================================
// 5. MANAJEMEN PROFIL & SOSIAL
// ==========================================
async function fetchProfile() {
    const { data } = await supabaseClient.from('profiles').select('*').eq('id', currentUser.id).single();
    if (data) { userProfile = data; } else { userProfile = { nickname: 'Player', avatar_url: '', bio: 'Halo! Salam kenal.' }; await supabaseClient.from('profiles').upsert({ id: currentUser.id, nickname: 'Player', avatar_url: '', bio: 'Halo! Salam kenal.' }); }
    if(!viewedUserId || viewedUserId === currentUser.id) {
        ...
        if (userProfile.avatar_url && userProfile.avatar_url !== "") { const pImg = document.getElementById('profile-img'); if(pImg) pImg.src = userProfile.avatar_url; }
    }
}

async function openUserProfile(userId) {
    viewedUserId = userId; switchTab('profile', null, false);
    document.getElementById('profile-loading').classList.remove('hidden'); document.querySelectorAll('#profile-logged-in').forEach(el => el.classList.add('hidden'));
    ...
    const inputAva = document.getElementById('btn-edit-avatar'); if(inputAva) inputAva.classList.toggle('hidden', !isOwn);
    const elVidTitle = document.getElementById('profile-video-title');
    if(elVidTitle) elVidTitle.innerHTML = isOwn ? `<i class="fas fa-grip-vertical mr-2 text-brand-info"></i> Video Saya` : `<i class="fas fa-grip-vertical mr-2 text-brand-info"></i> Video ${data?.nickname || ''}`;
    
    const userVideos = allVideosData.filter(v => v.user_id === userId);
    const elLikes = document.getElementById('profile-total-likes'); if (elLikes) elLikes.innerText = (userVideos.length * 3);

    renderProfileVideos(userId); fetchFollowStats(userId); checkBlockStatusUI();
    document.getElementById('profile-loading').classList.add('hidden'); document.querySelectorAll('#profile-logged-in').forEach(el => el.classList.remove('hidden'));
    if(!isOwn) history.pushState({ popup: 'user_profile' }, null, `#profile?id=${userId}`);
}

function viewUserProfile(userId) {
    if (!userId || userId === 'undefined') { showToast("Profil tidak dapat dibuka karena ID tidak ditemukan.", "error"); return; }
    if (blockedUsersList.includes(userId)) { showToast("Anda telah memblokir pengguna ini. Buka blokir untuk melihat profil.", "error"); return; }
    document.getElementById('comment-drawer').classList.remove('open'); cancelReply();
    const widget = document.getElementById('floating-widget'); if (widget && !widget.classList.contains('opacity-0')) widget.classList.add('opacity-0', 'pointer-events-none', 'translate-y-8', 'scale-95');
    openUserProfile(userId);
}

function kembaliDariProfil() {
    switchTab(tabSebelumnya, null, false); history.replaceState(null, null, '#' + tabSebelumnya);
    if (currentUser) {
        ...
        const elVidTitle = document.getElementById('profile-video-title'); if(elVidTitle) elVidTitle.innerHTML = `<i class="fas fa-grip-vertical mr-2 text-brand-info"></i> Video Saya`;

        renderProfileVideos(currentUser.id); fetchFollowStats(currentUser.id);
    } else {
        viewedUserId = null; document.querySelectorAll('#profile-logged-in').forEach(el => el.classList.add('hidden'));
        document.getElementById('profile-logged-out').classList.remove('hidden');
        const btnBack = document.getElementById('btn-back-profile'); if(btnBack) btnBack.classList.add('hidden');
    }
}
...
function openEditProfileModal() {
    ...
    document.getElementById('modal-edit-profile').classList.remove('hidden'); document.getElementById('modal-edit-profile').classList.add('flex');
    history.pushState({ popup: 'edit_profile' }, null, '#edit');
}

function closeEditProfileModal() {
    if(window.location.hash === '#edit') history.back();
    else { const m = document.getElementById('modal-edit-profile'); m.classList.add('hidden'); m.classList.remove('flex'); }
}

async function saveProfileInfo() {
    const btn = document.querySelector('button[onclick="saveProfileInfo()"]'); const originalText = btn.innerHTML;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Menyimpan...'; btn.disabled = true;
    try {
        const newNick = document.getElementById('edit-nick').value.trim(), newBio = document.getElementById('edit-bio').value.trim(), newPass = document.getElementById('edit-pass').value.trim();
        if(!newNick) { showToast("Nickname tidak boleh kosong!", "error"); return; }
        if (newPass) { const { error: errPass } = await supabaseClient.auth.updateUser({ password: newPass }); if (errPass) { showToast("Gagal ganti password: " + errPass.message, "error"); return; } }
        ...
        if (error) { if (error.code === '23505') throw new Error("Nickname tersebut sudah dipakai player lain. Silakan cari yang lain!"); throw error; }
        await fetchProfile(); updateUIForLoggedIn(); closeEditProfileModal(); showToast("Profil berhasil diperbarui!", "success");
    } catch (err) { showToast("Terjadi Kesalahan: " + err.message, "error"); } finally { btn.innerHTML = originalText; btn.disabled = false; }
}
...
async function handleAvatarUpload(event) {
    ...
    try {
        ...
        const base64data = await new Promise((resolve) => { const reader = new FileReader(); reader.readAsDataURL(compressedBlob); reader.onloadend = () => resolve(reader.result); });
        const response = await fetch('/api/upload-foto', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileBase64: base64data, userId: currentUser.id }) });
...
async function toggleFollow(targetUserId) {
    ...
    const btns = document.querySelectorAll('#btn-follow'); btns.forEach(btn => btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>');
    ...
    if (data) {
        ...
        btns.forEach(btn => { btn.innerText = 'IKUTI'; btn.classList.replace('bg-white/10', 'bg-brand-accent'); }); showToast("Batal mengikuti", "info");
    } else {
        await supabaseClient.from('follows').insert({ follower_id: currentUser.id, following_id: targetUserId });
        btns.forEach(btn => { btn.innerText = 'MENGIKUTI'; btn.classList.replace('bg-brand-accent', 'bg-white/10'); }); showToast("Berhasil mengikuti!", "success");
    }
...
async function feedToggleFollow(targetUserId, btnElement) {
    if (!currentUser) return openAuthModal();
    if (currentUser.id === targetUserId) { showToast("Ini video milikmu sendiri!", "info"); return; }
    const icon = btnElement.querySelector('i'); icon.className = 'fas fa-spinner fa-spin text-[10px]';
    try {
        await toggleFollow(targetUserId); 
        btnElement.classList.add('scale-0', 'opacity-0'); setTimeout(() => { btnElement.style.display = 'none'; }, 300);
...
async function showUserList(type) {
    ...
    const modal = document.getElementById('modal-user-list'); const container = document.getElementById('user-list-container'); const title = document.getElementById('user-list-title');
    title.innerText = type === 'mengikuti' ? 'Mengikuti' : 'Pengikut'; modal.classList.remove('hidden'); modal.classList.add('flex');
    ...
    if (profiles && profiles.length > 0) {
        container.innerHTML = profiles.map(p => {
            ...
            return `<div onclick="viewUserProfile('${p.id}'); closeUserList();" class="flex items-center p-3 hover:bg-white/5 cursor-pointer rounded-2xl transition-all border-b border-white/5 last:border-0"><img src="${ava}" class="w-10 h-10 rounded-full object-cover border border-white/10 shrink-0"><div class="ml-3"><h4 class="font-bold text-white text-xs">${p.nickname}</h4><p class="text-[10px] text-gray-400 truncate w-48">${p.bio || ''}</p></div></div>`;
        }).join('');
    }
}
function closeUserList() { document.getElementById('modal-user-list').classList.add('hidden'); document.getElementById('modal-user-list').classList.remove('flex'); }

async function fetchBlockedUsers() {
    if(!currentUser) return;
    try { const { data, error } = await supabaseClient.from('blocks').select('blocked_id').eq('blocker_id', currentUser.id); if(!error && data) { blockedUsersList = data.map(d => d.blocked_id); } } catch(e) { console.error("Gagal mengambil status blokir", e); }
}
...
async function toggleBlockUser(userId) {
    ...
    try {
        if(blockedUsersList.includes(userId)) {
            if(confirm("Buka blokir pengguna ini?")) {
                btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memproses...';
                await supabaseClient.from('blocks').delete().eq('blocker_id', currentUser.id).eq('blocked_id', userId);
                blockedUsersList = blockedUsersList.filter(id => id !== userId); showToast("Blokir dibuka", "success"); checkBlockStatusUI();
            }
        } else {
            if(confirm("Blokir pengguna ini? Anda tidak akan melihat video dan pesannya.")) {
                btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memproses...';
                await supabaseClient.from('blocks').insert({ blocker_id: currentUser.id, blocked_id: userId });
                blockedUsersList.push(userId); showToast("Pengguna diblokir", "info"); checkBlockStatusUI();
                allVideosData = allVideosData.filter(v => v.user_id !== userId); if(document.getElementById('sosial').classList.contains('active')) loadVideos();
            }
        }
...
function checkBlockStatusUI() {
    const btn = document.getElementById('btn-block-user'); if(!btn || !viewedUserId) return;
    if(blockedUsersList.includes(viewedUserId)) { btn.innerHTML = '<i class="fas fa-unlock mr-1"></i> Buka Blokir'; btn.className = 'w-full bg-gray-500/10 border border-gray-500/20 py-2 rounded-xl text-[10px] font-bold text-gray-400 hover:bg-gray-500/20 transition-colors mt-1'; } 
    else { btn.innerHTML = '<i class="fas fa-ban mr-1"></i> Blokir Pengguna'; btn.className = 'w-full bg-red-500/10 border border-red-500/20 py-2 rounded-xl text-[10px] font-bold text-red-500 hover:bg-red-500/20 transition-colors mt-1'; }
}

// ==========================================
// 6. VIDEO FEED (MAIN & PROFIL)
// ==========================================
async function loadVideos() {
    ...
    try {
        ...
        const res = await fetch('/api/get-videos'); let dataDariSheet = await res.json();
        dataDariSheet = dataDariSheet.map(v => { v.id = v.id || v.video_id || v.ID || 'vid_' + Math.random().toString(36).substr(2, 9); return v; });
        newUploads.forEach(newVid => { newVid.id = newVid.id || newVid.video_id; if (!dataDariSheet.find(v => v.id === newVid.id)) dataDariSheet.push(newVid); });
        
        dataDariSheet = dataDariSheet.filter(v => !blockedUsersList.includes(v.user_id));
        for (let i = dataDariSheet.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [dataDariSheet[i], dataDariSheet[j]] = [dataDariSheet[j], dataDariSheet[i]]; }
        
        allVideosData = dataDariSheet; currentVideoIndex = 0; container.innerHTML = ''; 
        renderProfileVideos();
            ...
            if (targetCount === 0) { clearTimeout(safetyTimeout); finishLoading(); } 
                ...
                videosInDOM.forEach((vid, index) => {
                    if (index < targetCount) {
                        ...
                        const onVideoReady = () => { if (isResolved) return; readyCount++; const progressPercent = 80 + ((readyCount / targetCount) * 20); fakeProgress.style.width = `${progressPercent}%`; if (readyCount >= targetCount) { clearTimeout(safetyTimeout); finishLoading(); } };
...
function renderVideoBatch() {
    ...
    if (currentVideoIndex >= allVideosData.length) currentVideoIndex = 0; 
    ...
    const htmlString = nextBatch.map((vid) => `
        <div class="snap-start w-full h-full flex-shrink-0 relative group flex items-center justify-center overflow-hidden bg-brand-dark">
            ...
            <div class="time-indicator absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white font-extrabold text-4xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)] opacity-0 transition-opacity z-[60] pointer-events-none tracking-wider bg-black/40 px-6 py-2 rounded-2xl backdrop-blur-sm">
                <span class="time-current">00:00</span> <span class="text-white/50 text-2xl mx-1">/</span> <span class="time-total text-white/70">00:00</span> </div>
            <div class="absolute bottom-0 left-0 w-full h-3 z-50 cursor-pointer group touch-none flex flex-col justify-end pb-1" onpointerdown="startSeek(event, this)" onpointermove="doSeek(event, this)" onpointerup="endSeek(event, this)" onpointercancel="endSeek(event, this)">
                <div class="w-full h-1 bg-white/30 relative"><div class="progress-fill h-full bg-white w-0 relative pointer-events-none transition-all duration-75 ease-linear"><div class="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full scale-0 group-hover:scale-100 transition-transform shadow-md"></div></div></div>
            </div>
            <div class="absolute bottom-0 left-0 w-full px-5 pb-8 flex justify-between items-end z-20 pointer-events-none">
                <div class="text-white drop-shadow-lg w-[75%] pointer-events-auto">
                    <p onclick="openUserProfile('${vid.user_id}')" class="font-bold text-[15px] cursor-pointer hover:text-brand-info shadow-black drop-shadow-md mb-1.5">@${vid.nickname || "Player"}</p>
                ...
                <div class="flex flex-col gap-4 items-center pointer-events-auto mb-6 sm:mb-10 mr-1">
                    <div class="relative mb-2 cursor-pointer hover:scale-105 transition-transform" onclick="openUserProfile('${vid.user_id}')">
                        <img src="${vid.avatar_url || 'https://ui-avatars.com/api/?name=User&background=1A1133&color=fff'}" loading="lazy" class="w-11 h-11 rounded-full object-cover border-[1.5px] border-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                        <button id="feed-follow-btn-${vid.user_id}" onclick="event.stopPropagation(); feedToggleFollow('${vid.user_id}', this)" class="absolute -bottom-2.5 left-1/2 -translate-x-1/2 bg-[#FF007A] text-white rounded-full w-[22px] h-[22px] flex items-center justify-center border-[1.5px] border-brand-dark drop-shadow-md active:scale-90 transition-transform z-30"><i class="fas fa-plus text-[10px]"></i></button>
                    </div>
                    <div class="like-container flex flex-col items-center gap-0.5" data-vid="${vid.id}"><button onclick="likeVideo('${vid.id}', this)" class="hover:scale-110 transition-transform active:scale-90"><i class="fas fa-heart text-[32px] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"></i></button><span class="like-count-display text-white text-[12px] font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">0</span></div>
                    <div class="comment-count-container flex flex-col items-center gap-0.5" data-vid="${vid.id}"><button onclick="openComments('${vid.id}')" class="hover:scale-110 transition-transform active:scale-90"><i class="fas fa-comment-dots text-[32px] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" style="transform: scaleX(-1);"></i></button><span class="comment-count-display text-white text-[12px] font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">0</span></div>
                    <div class="flex flex-col items-center gap-0.5 mt-1"><button onclick="shareVideo('${vid.id}', this)" class="hover:scale-110 transition-transform active:scale-90"><i class="fas fa-share text-[30px] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"></i></button><span class="text-white text-[11px] font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">Share</span></div>
                </div>
            </div>
        </div>`).join('');

    container.insertAdjacentHTML('beforeend', htmlString); currentVideoIndex += BATCH_SIZE;
    const videoActions = container.querySelectorAll('.snap-start:not(.data-loaded)');
    videoActions.forEach((card) => { card.classList.add('data-loaded'); const vidId = card.querySelector('.like-container').dataset.vid; updateLikeCountUI(vidId, card.querySelector('.like-container')); updateCommentCountUI(vidId, card.querySelector('.comment-count-container')); });
    ...
    unobservedVideos.forEach((v, i) => {
        ...
        if (i === unobservedVideos.length - 1) {
            const lastVideoObserver = new IntersectionObserver(entries => { if(entries[0].isIntersecting) { lastVideoObserver.disconnect(); renderVideoBatch(); } }, { threshold: 0.1 });
            lastVideoObserver.observe(v.closest('.snap-start'));
        }
    });
}

// Fitur Optimasi V6 - Observer Anti Lag
function setupVideoObserver() {
    if (typeof obs !== 'undefined' && obs) obs.disconnect();
    if (!document.getElementById('gpu-hack')) { const style = document.createElement('style'); style.id = 'gpu-hack'; style.innerHTML = `.snap-start { transform: translateZ(0); will-change: transform, opacity; content-visibility: auto; } video { will-change: contents; }`; document.head.appendChild(style); }
    window.videoClearTimers = window.videoClearTimers || new Map(); window.videoPlayTimers = window.videoPlayTimers || new Map(); 
        ...
        es.forEach(e => { 
            ...
            if (e.isIntersecting && !isFloatingOpen) {
                ...
                const playTimerId = setTimeout(() => {
                    ...
                    requestAnimationFrame(() => { const playPromise = video.play(); if (playPromise !== undefined) { playPromise.catch(err => {}); } video.classList.remove('opacity-0'); });
                    const card = video.closest('.snap-start');
                    if (card && card.nextElementSibling) { const nextVid = card.nextElementSibling.querySelector('video'); if (nextVid && nextVid.dataset.savedSrc && !nextVid.src) { nextVid.src = nextVid.dataset.savedSrc; nextVid.dataset.savedSrc = ''; nextVid.setAttribute('preload', 'auto'); } }
                }, 100); 
                ...
                const timerId = setTimeout(() => {
                    const hapusMemori = () => {
                        ...
                        if (currentUrl && currentUrl.length > 5) { video.dataset.savedSrc = currentUrl; video.removeAttribute('src'); if (video.querySelector('source')) video.querySelector('source').removeAttribute('src'); video.removeAttribute('preload'); video.load(); }
    ...
    const videos = document.querySelectorAll('.video-player, .float-video-player'); videos.forEach(v => obs.observe(v));
}

async function renderProfileVideos(targetUserId = null) {
    const grid = document.getElementById('profile-video-grid'); if (!grid) return;
    const uidToRender = targetUserId || (currentUser ? currentUser.id : null); if (!uidToRender) { grid.innerHTML = ''; return; }
    ...
    if (allVideosData.length === 0) {
        try {
            ...
            dataDariSheet = dataDariSheet.map(v => { v.id = v.id || v.video_id || v.ID || 'vid_' + Math.random().toString(36).substr(2, 9); return v; });
            newUploads.forEach(newVid => { newVid.id = newVid.id || newVid.video_id; if (!dataDariSheet.find(v => v.id === newVid.id)) dataDariSheet.push(newVid); });
            dataDariSheet = dataDariSheet.filter(v => !blockedUsersList.includes(v.user_id)); allVideosData = dataDariSheet;
        } catch(e) { grid.innerHTML = '<p class="col-span-3 text-center text-xs text-red-500 py-4">Gagal memuat video profil.</p>'; return; }
    }
    ...
    if (reversedVideos.length === 0) { grid.innerHTML = '<div class="col-span-3 text-center text-xs text-gray-500 py-8 bg-black/20 rounded-xl border border-white/5"><i class="fas fa-film text-2xl mb-2 opacity-50"></i><br>Belum ada video yang diupload.</div>'; return; }

    grid.innerHTML = reversedVideos.map((vid, index) => {
        return `<div class="aspect-[9/16] bg-black relative rounded-md overflow-hidden border border-white/10 group cursor-pointer" onclick="openProfileFeed('${vid.user_id}', ${index})"><video class="w-full h-full object-cover" preload="metadata"><source src="${vid.video_url}" type="video/mp4"></video><div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><i class="fas fa-play text-white text-xl"></i></div></div>`;
    }).join('');
}

function openProfileFeed(userId, startIndex) {
    currentProfileVideos = allVideosData.filter(v => v.user_id === userId); if(currentProfileVideos.length === 0) return;
    currentProfileVideos = [...currentProfileVideos].reverse(); profileFeedIndex = parseInt(startIndex) || 0;
    const container = document.getElementById('floating-feed-container'); container.innerHTML = ''; container.scrollTop = 0; 
    document.querySelectorAll('.video-player').forEach(v => v.pause());
    document.getElementById('floating-video-player').classList.remove('hidden'); document.getElementById('floating-video-player').classList.add('flex');
    if(floatObs) floatObs.disconnect(); setupFloatVideoObserver(); renderProfileVideoBatch(); history.pushState({ popup: 'floating_video' }, null);
}

function renderProfileVideoBatch() {
    const container = document.getElementById('floating-feed-container'); if (profileFeedIndex >= currentProfileVideos.length) return;
    const nextBatch = currentProfileVideos.slice(profileFeedIndex, profileFeedIndex + 3); if (nextBatch.length === 0) return;
    ...
    const htmlString = nextBatch.map((vid) => `
        <div class="snap-start w-full h-full flex-shrink-0 relative flex items-center justify-center overflow-hidden bg-black/95 px-0 sm:px-4 py-0 sm:py-6">
            <div class="w-full max-w-sm aspect-[9/16] relative bg-brand-dark mx-auto h-full sm:h-auto sm:rounded-3xl overflow-hidden shadow-2xl">
                <div class="absolute inset-0 flex items-center justify-center z-0"><div class="w-12 h-12 border-4 border-brand-accent/20 border-t-brand-accent rounded-full animate-spin"></div></div>
                <video class="absolute inset-0 m-auto w-full h-full object-cover float-video-player transition-opacity duration-500 opacity-0 z-10" onloadeddata="this.classList.remove('opacity-0')" loop ${isGlobalMuted ? 'muted' : ''} playsinline preload="metadata" ontimeupdate="updateVideoProgress(this)" onclick="handleFloatVideoClick(event, this, '${vid.id}')" onerror="handleVideoError(this)"><source src="${vid.video_url}" type="video/mp4"></video>
                <div class="time-indicator absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white font-extrabold text-4xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)] opacity-0 transition-opacity z-[60] pointer-events-none tracking-wider bg-black/40 px-6 py-2 rounded-2xl backdrop-blur-sm"><span class="time-current">00:00</span> <span class="text-white/50 text-2xl mx-1">/</span> <span class="time-total text-white/70">00:00</span> </div>
                <div class="absolute bottom-0 left-0 w-full h-3 z-50 cursor-pointer group touch-none flex flex-col justify-end pb-1" onpointerdown="startSeek(event, this)" onpointermove="doSeek(event, this)" onpointerup="endSeek(event, this)" onpointercancel="endSeek(event, this)"><div class="w-full h-1 bg-white/30 relative"><div class="progress-fill h-full bg-white w-0 relative pointer-events-none transition-all duration-75 ease-linear"><div class="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full scale-0 group-hover:scale-100 transition-transform shadow-md"></div></div></div></div>
                <div class="absolute bottom-0 left-0 w-full p-6 flex justify-between items-end z-20 pointer-events-none bg-gradient-to-t from-black/80 to-transparent pb-10 sm:pb-6">
                    ...
                    <div class="flex flex-col gap-4 items-center pointer-events-auto mb-2 mr-1">
                        ...
                        <div class="comment-count-container flex flex-col items-center gap-0.5" data-vid="${vid.id}"><button onclick="openComments('${vid.id}')" class="hover:scale-110 transition-transform active:scale-90"><i class="fas fa-comment-dots text-[32px] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" style="transform: scaleX(-1);"></i></button><span class="comment-count-display text-white text-[12px] font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">0</span></div>
                        <div class="flex flex-col items-center gap-0.5 mt-1"><button onclick="shareVideo('${vid.id}', this)" class="hover:scale-110 transition-transform active:scale-90"><i class="fas fa-share text-[30px] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"></i></button><span class="text-white text-[11px] font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">Share</span></div>
                        ${currentUser && vid.user_id === currentUser.id ? `<button onclick="deleteVideo('${vid.id}')" class="hover:scale-110 transition-transform mt-3 active:scale-90"><div class="bg-black/50 p-2 rounded-full border border-white/20 backdrop-blur-sm"><i class="fas fa-trash text-red-500 text-xl drop-shadow-md"></i></div></button>` : ''}
                    </div>
                </div>
            </div>
        </div>`).join('');

    container.insertAdjacentHTML('beforeend', htmlString); profileFeedIndex += 3;
    const videoActions = container.querySelectorAll('.snap-start:not(.data-loaded)');
    videoActions.forEach((card) => { card.classList.add('data-loaded'); const vidId = card.querySelector('.like-container').dataset.vid; updateLikeCountUI(vidId, card.querySelector('.like-container')); updateCommentCountUI(vidId, card.querySelector('.comment-count-container')); });
    ...
    unobservedVideos.forEach((v, i) => {
        ...
        if (i === unobservedVideos.length - 1) {
            const lastVideoObserver = new IntersectionObserver(entries => { if(entries[0].isIntersecting) { lastVideoObserver.disconnect(); renderProfileVideoBatch(); } }, { threshold: 0.1 });
            lastVideoObserver.observe(v.closest('.snap-start'));
        }
    });
}
        ...
        es.forEach(e => {
            if (e.isIntersecting) {
                ...
                for(let j = 0; j < 2; j++) { 
                    ...
                    if(currentContainer) { const nextVid = currentContainer.querySelector('video'); if(nextVid && nextVid.getAttribute('preload') !== 'auto') nextVid.setAttribute('preload', 'auto'); }
                }
            } else { e.target.pause(); }
        });
    }, { threshold: 0.6 });
...
function handleFloatVideoClick(event, videoElement, vidId) {
    const now = Date.now(); const TIMESLOT = 300; 
    if (now - lastTap < TIMESLOT) { const card = videoElement.closest('.snap-start'); const likeBtn = card.querySelector('.like-container button'); likeVideo(vidId, likeBtn); createHeartAt(event); } 
    else { setTimeout(() => { if (Date.now() - lastTap >= TIMESLOT) { toggleGlobalAudio(); } }, TIMESLOT); }
    lastTap = now;
}

function closeFloatingVideo() { const p = document.getElementById('floating-video-player'); p.classList.add('hidden'); p.classList.remove('flex'); document.getElementById('floating-feed-container').innerHTML = ''; if(floatObs) floatObs.disconnect(); }
...
async function likeVideo(videoId, btn) {
    ...
    const icon = btn.querySelector('i'); icon.classList.replace('text-white', 'text-brand-accent'); icon.classList.add('animate-ping'); setTimeout(() => icon.classList.remove('animate-ping'), 500);

    try {
        const { error } = await supabaseClient.from('video_likes').insert({ video_id: videoId, user_id: currentUser.id });
        if (!error) { localStorage.setItem(`liked_${videoId}`, 'true'); updateLikeCountUI(videoId, btn.closest('.like-container') || document.getElementById('float-like-container')); } else { throw error; }
    } catch (e) { localStorage.removeItem(`liked_${videoId}`); icon.classList.replace('text-brand-accent', 'text-white'); showToast("Gagal menyukai video.", "error"); }
}

async function deleteVideo(vidId) {
    const hapus = await customPrompt("Ketik 'HAPUS' jika ingin menghapus video ini:");
    if(hapus === 'HAPUS') { allVideosData = allVideosData.filter(v => v.id !== vidId); closeFloatingVideo(); renderProfileVideos(); showToast("Video berhasil dihapus", "success"); }
}
...
function shareVideo(vidId, btn) {
    ...
    navigator.clipboard.writeText(link).then(() => { showToast("Link video disalin ke clipboard!", "success"); const icon = btn.querySelector('i'); icon.classList.replace('fa-share', 'fa-check'); icon.classList.add('text-brand-success'); setTimeout(() => { icon.classList.replace('fa-check', 'fa-share'); icon.classList.remove('text-brand-success'); }, 2000); }).catch(() => { showToast("Gagal menyalin link otomatis", "error"); });
}

function toggleGlobalAudio() { isGlobalMuted = !isGlobalMuted; document.querySelectorAll('.video-player, .float-video-player').forEach(v => v.muted = isGlobalMuted); if(isGlobalMuted) { showToast("Mute: Suara Dimatikan", "info"); } else { showToast("Unmute: Suara Dinyalakan", "success"); } }

function handleVideoError(videoElement) { const container = videoElement.closest('.snap-start'); if (container) container.remove(); }
function createHeartAt(event) { const heart = document.createElement('i'); heart.className = 'fas fa-heart heart-pop'; const x = event.clientX || (event.touches && event.touches[0].clientX) || (window.innerWidth / 2); const y = event.clientY || (event.touches && event.touches[0].clientY) || (window.innerHeight / 2); heart.style.left = `${x}px`; heart.style.top = `${y}px`; document.body.appendChild(heart); setTimeout(() => heart.remove(), 800); }

// ==========================================
// 7. DRAG & GESER PROGRESS BAR VIDEO
// ==========================================
...
function updateVideoProgress(video) {
    ...
    if (!video.progressFill) { const container = video.closest('.snap-start'); if (container) video.progressFill = container.querySelector('.progress-fill'); }
    if (video.progressFill && video.duration) { requestAnimationFrame(() => { const percent = (video.currentTime / video.duration) * 100; video.progressFill.style.width = `${percent}%`; }); }
}

function startSeek(e, containerElement) { const video = containerElement.closest('.snap-start').querySelector('video'); if (!video) return; video.isDragging = true; const timeIndicator = containerElement.closest('.snap-start').querySelector('.time-indicator'); if (timeIndicator) timeIndicator.classList.replace('opacity-0', 'opacity-100'); updateSeek(e, containerElement, video); }
function doSeek(e, containerElement) { const video = containerElement.closest('.snap-start').querySelector('video'); if (!video || !video.isDragging) return; updateSeek(e, containerElement, video); }
function endSeek(e, containerElement) { const video = containerElement.closest('.snap-start').querySelector('video'); if (!video) return; if (video.isDragging) { updateSeek(e, containerElement, video); video.isDragging = false; } const timeIndicator = containerElement.closest('.snap-start').querySelector('.time-indicator'); if (timeIndicator) timeIndicator.classList.replace('opacity-100', 'opacity-0'); }

function updateSeek(e, containerElement, video) {
    const rect = containerElement.getBoundingClientRect(); let x = e.clientX; if (x === undefined && e.touches && e.touches.length > 0) x = e.touches[0].clientX; if (x === undefined) return;
    let pos = (x - rect.left) / rect.width; pos = Math.max(0, Math.min(1, pos)); 
    const fill = containerElement.querySelector('.progress-fill'); if (fill) fill.style.width = `${pos * 100}%`;
    if (video.duration) { video.currentTime = pos * video.duration; const timeIndicator = containerElement.closest('.snap-start').querySelector('.time-indicator'); if (timeIndicator) { timeIndicator.querySelector('.time-current').innerText = formatTime(video.currentTime); timeIndicator.querySelector('.time-total').innerText = formatTime(video.duration); } }
}

// ==========================================
// 8. KOMENTAR & LIKE VIDEO UI
// ==========================================
function openComments(id) { history.pushState({ popup: 'comments' }, null, '#comments'); activeVideoId = id; document.getElementById('comment-drawer').classList.add('open'); loadComments(id); }
function closeComments() { if (window.location.hash === '#comments') { history.back(); } else { document.getElementById('comment-drawer').classList.remove('open'); cancelReply(); } }
...
async function loadComments(videoId, silent = false) {
    ...
    try {
        ...
        if (data && data.length > 0) {
            const mainComments = data.filter(c => !c.parent_id); const replies = data.filter(c => c.parent_id);
            if(mainComments.length === 0) { list.innerHTML = '<p class="text-center text-xs text-gray-500 italic mt-10">Belum ada komentar.</p>'; return; }
            let htmlOutput = ''; mainComments.forEach(c => { const childReplies = replies.filter(r => r.parent_id == c.id); htmlOutput += buildCommentHTML(c, childReplies); });
            list.innerHTML = htmlOutput; setTimeout(() => { data.forEach(c => fetchCommentLikes(c.id)); }, 50);
        } else { list.innerHTML = '<p class="text-center text-xs text-gray-500 italic mt-10">Jadilah yang pertama berkomentar!</p>'; }
...
function buildCommentHTML(comment, replies = []) {
    ...
    let html = `<div class="flex items-start gap-3 mb-5" id="comment-box-${comment.id}"><img src="${ava}" onclick="viewUserProfile('${comment.user_id}')" class="w-8 h-8 rounded-full object-cover border border-white/10 shrink-0 mt-1 cursor-pointer hover:opacity-80 transition-opacity"><div class="flex-1"><div class="flex justify-between items-start"><div class="pr-2"><b onclick="viewUserProfile('${comment.user_id}')" class="text-brand-info text-[11px] cursor-pointer hover:underline">${comment.nickname}</b><p class="text-gray-200 text-xs mt-0.5 leading-relaxed break-words">${formatCaption(comment.message)}</p><div class="flex items-center gap-2 mt-1.5"><span class="text-[9px] text-gray-600">${timeAgo(comment.created_at)}</span><button onclick="setReply('${comment.id}', '${comment.nickname}')" class="text-[10px] text-gray-400 font-bold hover:text-white px-2">Balas</button> ${delBtn} </div></div><div class="flex flex-col items-center gap-1 ml-1 shrink-0"><button onclick="likeComment('${comment.id}', this)" class="text-gray-500 hover:text-brand-accent transition-colors active:scale-75"><i class="fas fa-heart text-sm ${localStorage.getItem('comment_liked_'+comment.id) ? 'text-brand-accent' : ''}"></i></button><span class="text-[9px] text-gray-500 font-medium comment-like-count" data-cid="${comment.id}">0</span></div></div>`;

    if (replies.length > 0) {
        html += `<div class="mt-2"><button onclick="toggleReplies('${comment.id}')" class="text-[10px] text-brand-info font-bold flex items-center gap-1.5 active:scale-95 transition-transform"><span class="w-5 h-[1px] bg-brand-info/50 inline-block"></span> Lihat ${replies.length} balasan <i class="fas fa-chevron-down text-[8px] transition-transform duration-300" id="icon-reply-${comment.id}"></i></button><div id="replies-${comment.id}" class="hidden mt-3 space-y-4 pl-3 border-l border-white/10 relative">`;
        replies.forEach(r => {
            const rAva = r.avatar_url || `https://ui-avatars.com/api/?name=${r.nickname || 'User'}&background=1A1133&color=fff`; const isRMe = currentUser && currentUser.id === r.user_id; const rDelBtn = isRMe ? `<button onclick="deleteComment('${r.id}')" class="text-red-500/50 hover:text-red-500 transition-colors px-2"><i class="fas fa-times text-[10px]"></i></button>` : '';
            html += `<div class="flex items-start gap-2 relative" id="comment-box-${r.id}"><img src="${rAva}" onclick="viewUserProfile('${r.user_id}')" class="w-6 h-6 rounded-full object-cover border border-white/10 shrink-0 mt-0.5 cursor-pointer hover:opacity-80 transition-opacity"><div class="flex-1"><div class="flex justify-between items-start"><div class="pr-2"><b onclick="viewUserProfile('${r.user_id}')" class="text-brand-purple text-[10px] cursor-pointer hover:underline">${r.nickname}</b><p class="text-gray-300 text-[11px] mt-0.5 leading-relaxed break-words">${formatCaption(r.message)}</p><div class="flex items-center gap-2 mt-1"><button onclick="setReply('${comment.id}', '${r.nickname}')" class="text-[9px] text-gray-500 font-bold hover:text-white pr-2">Balas</button> ${rDelBtn}</div></div><div class="flex flex-col items-center gap-1 ml-1 shrink-0"><button onclick="likeComment('${r.id}', this)" class="text-gray-500 hover:text-brand-accent transition-colors active:scale-75"><i class="fas fa-heart text-[11px] ${localStorage.getItem('comment_liked_'+r.id) ? 'text-brand-accent' : ''}"></i></button><span class="text-[8px] text-gray-500 comment-like-count" data-cid="${r.id}">0</span></div></div></div></div>`;
        });
        html += `</div></div>`;
    }
...
async function deleteComment(cid) {
    ...
    if (hapus) {
        const cBox = document.getElementById('comment-box-' + cid); if(cBox) cBox.style.display = 'none';
        try { const { error } = await supabaseClient.from('comments').delete().eq('id', cid); if(error) throw error; showToast("Komentar dihapus.", "success"); } 
        catch(e) { showToast("Gagal menghapus komentar dari database.", "error"); if(cBox) cBox.style.display = 'flex'; }
    }
}
...
function setReply(parentId, nickname) {
    ...
    document.getElementById('reply-indicator').classList.remove('hidden'); document.getElementById('replying-to-name').innerText = '@' + nickname;
    const input = document.getElementById('comment-input'); input.focus(); if(nickname) input.value = `@${nickname} `;
}

function cancelReply() { replyingToId = null; replyingToName = null; document.getElementById('reply-indicator').classList.add('hidden'); const input = document.getElementById('comment-input'); if (input.value.startsWith('@')) input.value = ''; }

async function sendComment(event) {
    if (!currentUser) return openAuthModal(); 
    const msgInput = document.getElementById('comment-input'); const msg = msgInput.value.trim(); if (!msg) { showToast("Komentar tidak boleh kosong!", "error"); return; }
    const btn = event.currentTarget; const iconAsli = btn.innerHTML; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>'; btn.disabled = true;
    ...
    try {
        ...
        if (response.ok) {
            msgInput.value = ''; await loadComments(activeVideoId, true);
            const container = document.querySelector(`.comment-count-container[data-vid="${activeVideoId}"]`); if(container) updateCommentCountUI(activeVideoId, container);
            const floatComment = document.getElementById('float-comment-container'); if (floatComment && floatComment.dataset.vid === activeVideoId) { updateCommentCountUI(activeVideoId, floatComment); }
            cancelReply();
...
async function fetchCommentLikes(cid) {
    try { const { count, error } = await supabaseClient.from('comment_likes').select('*', { count: 'exact', head: true }).eq('comment_id', cid); const span = document.querySelector(`.comment-like-count[data-cid="${cid}"]`); if(span && !error) span.innerText = count || 0; } catch(e) { console.error("Gagal mengambil like komentar", e); }
}
...
async function likeComment(cid, btn) {
    ...
    const icon = btn.querySelector('i'); icon.classList.remove('text-gray-500'); icon.classList.add('text-brand-accent', 'animate-ping'); setTimeout(() => icon.classList.remove('animate-ping'), 500);
    try {
    ...
    } catch(e) { localStorage.removeItem('comment_liked_'+cid); icon.classList.add('text-gray-500'); icon.classList.remove('text-brand-accent'); showToast("Gagal menyukai komentar.", "error"); }
}
...
async function updateLikeCountUI(videoId, containerDiv) {
    ...
    try {
        ...
        const countSpan = containerDiv.querySelector('.like-count-display'); if (countSpan && !error) countSpan.innerText = count || 0;
...
async function updateCommentCountUI(videoId, containerDiv) {
    try { const response = await fetch(`/api/comment?video_id=${videoId}`); const data = await response.json(); const countSpan = containerDiv.querySelector('.comment-count-display'); if (countSpan && Array.isArray(data)) countSpan.innerText = data.length; } catch(e) {}
}
...
function handleVideoSelect(input) {
    ...
    const placeholder = document.getElementById('upload-placeholder'); const previewContainer = document.getElementById('video-preview-container'); const previewVideo = document.getElementById('video-preview-element'); const fileNameInfo = document.getElementById('nama-file-info');
    if (file) {
        const url = URL.createObjectURL(file); previewVideo.src = url; previewVideo.play();
        placeholder.classList.add('opacity-0');
        setTimeout(() => { placeholder.classList.add('hidden'); previewContainer.classList.remove('hidden'); previewContainer.classList.add('flex', 'animate-fade-in'); fileNameInfo.innerText = file.name; }, 300);
    }
}

function closeUploadModal() {
    if (window.location.hash === '#upload') history.back();
    const m = document.getElementById('modal-upload'); m.classList.add('hidden'); m.classList.remove('flex');
    document.getElementById('upload-placeholder').classList.remove('hidden', 'opacity-0'); document.getElementById('video-preview-container').classList.add('hidden'); document.getElementById('video-preview-element').src = '';
}

async function prosesUploadVideo() {
    if (!currentUser) return openAuthModal();
    const fileInput = document.getElementById('input-video-file'); const captionInput = document.getElementById('input-video-caption'); const file = fileInput.files[0];
    if (!file) return showToast("Pilih video dulu!", "error");

    const btn = document.querySelector('button[onclick="prosesUploadVideo()"]'); btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>'; isUploading = true;
    try {
        const configRes = await fetch('/api/get-config'); const config = await configRes.json();
        if (!config.gasUrl) throw new Error("Link GAS tidak ditemukan di config");
        const resUrl = await fetch(`/api/upload-url?filename=${encodeURIComponent(file.name)}&filetype=${file.type}`); const dataUrl = await resUrl.json();
        await fetch(dataUrl.uploadUrl, { method: 'PUT', body: file, headers: { 'Content-Type': file.type } });

        const spreadsheetPayload = {
            ID_Video: 'vid_' + Date.now(), URL_Video: dataUrl.finalVideoUrl,
            id: 'vid_' + Date.now(), video_url: dataUrl.finalVideoUrl, caption: captionInput.value || "", nickname: userProfile?.nickname || "Player", avatar_url: userProfile?.avatar_url || "", user_id: currentUser.id, created_at: new Date().toISOString()
        };
        ...
        fileInput.value = ''; captionInput.value = ''; closeUploadModal(); allVideosData = []; 
        if (tabSebelumnya && tabSebelumnya !== 'sosial' && tabSebelumnya !== 'upload') { switchTab(tabSebelumnya); } else { switchTab('sosial'); }
        loadVideos();
    } catch (err) { showToast("Error: " + err.message, "error"); } finally { btn.disabled = false; btn.innerHTML = '<i class="fas fa-paper-plane mr-2"></i> Posting Video'; isUploading = false; }
}

// ==========================================
// 10. STATUS / STORY LOGIC
// ==========================================
async function loadStories() {
    ...
    try {
        ...
        const myStoryRing = document.getElementById('my-story-ring'); const myStatusText = document.getElementById('my-status-text');
        ...
        for (const uid in groupedStories) {
            ...
            const user = groupedStories[uid]; const ava = user.avatar || `https://ui-avatars.com/api/?name=${user.nickname}`; const latestTime = timeAgo(user.stories[0].created_at);
            html += `<div class="flex items-center p-2 bg-brand-card/50 hover:bg-white/5 cursor-pointer rounded-2xl border border-white/5 transition-all mb-2" onclick="viewStory('${user.user_id}', '${user.nickname}', '${ava}')"><div class="relative shrink-0 story-ring"><img src="${ava}" class="w-11 h-11 rounded-full object-cover border-2 border-brand-card"></div><div class="ml-3 flex-1"><h4 class="font-bold text-white text-[13px]">${user.nickname}</h4><p class="text-[10px] text-brand-info">${latestTime}</p></div></div>`;
        }
...
async function handleUploadStory(event) {
    ...
    const captionStory = await customPrompt("Tulis caption untuk status ini:", ""); if (captionStory === null) return; 
    showToast("Mengunggah status...", "info");

    try {
        const resUrl = await fetch(`/api/upload-url?filename=${encodeURIComponent('story_'+Date.now())}&filetype=${encodeURIComponent(file.type)}`); const dataUrl = await resUrl.json();
        ...
        if (error) throw error; showToast("Status berhasil diperbarui!", "success"); loadStories(); 
...
async function viewStory(userId, name, avatar) {
    ...
    const modal = document.getElementById('story-viewer-modal'); modal.classList.remove('hidden'); modal.classList.add('flex'); setTimeout(() => modal.classList.remove('opacity-0'), 10);
    playStory(0);
}
...
function playStory(index) {
    ...
    const progressBar = document.getElementById('story-progress'); progressBar.style.transition = 'none'; progressBar.style.width = '0%';
    clearTimeout(currentStoryTimer);
    
    setTimeout(() => { progressBar.style.transition = 'width 5s linear'; progressBar.style.width = '100%'; currentStoryTimer = setTimeout(() => { nextStory(); }, 5000); }, 50);
}
...
function closeStoryViewer() { clearTimeout(currentStoryTimer); const modal = document.getElementById('story-viewer-modal'); modal.classList.add('opacity-0'); setTimeout(() => { modal.classList.remove('flex'); modal.classList.add('hidden'); }, 300); }

// ==========================================
// 11. CHAT, INBOX, DAN PRESENCE
// ==========================================
async function initPresence() {
    if (!currentUser || presenceChannel) return;
    presenceChannel = supabaseClient.channel('global_presence', { config: { presence: { key: currentUser.id } } });
    presenceChannel.on('presence', { event: 'sync' }, () => {
        const newState = presenceChannel.presenceState(); onlineUsersMap.clear();
        for (const id in newState) { onlineUsersMap.set(id, true); }
        updatePresenceUI();
...
function updatePresenceUI() {
    if (activeChatUserId) {
        const statusEl = document.getElementById('active-chat-status'); const dotEl = document.getElementById('active-chat-online-dot'); const isOnline = onlineUsersMap.has(activeChatUserId);
        if (statusEl) { statusEl.innerText = isOnline ? 'Online' : 'Offline'; statusEl.className = isOnline ? 'text-[9px] text-brand-info font-bold' : 'text-[9px] text-gray-500'; }
        if (dotEl) { isOnline ? dotEl.classList.remove('hidden') : dotEl.classList.add('hidden'); }
    }
}
...
async function checkGlobalUnreadMessages() {
    ...
    try {
        ...
        if(badge) { if(!error && count && count > 0) { badge.innerText = count > 99 ? '99+' : count; badge.classList.remove('hidden'); } else { badge.classList.add('hidden'); } }
...
function initGlobalMessageListener() {
    ...
    globalMessageSubscription = supabaseClient.channel('global_messages').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `receiver_id=eq.${currentUser.id}` }, payload => {
        ...
        if (activeChatUserId !== payload.new.sender_id && !blockedUsersList.includes(payload.new.sender_id)) {
            ...
            if ("Notification" in window && Notification.permission === "granted" && document.hidden) { new Notification("AU2Hub", { body: "Anda menerima pesan baru!", icon: "/app-icon-192.png" }); }
        }
        const widget = document.getElementById('floating-widget'); const chatList = document.getElementById('chat-list-view');
...
async function searchUsersForChat(query) {
    ...
    if (data && data.length > 0 && container) {
        ...
        data.forEach(p => {
            const ava = p.avatar_url || `https://ui-avatars.com/api/?name=${p.nickname}&background=1A1133&color=fff`;
            html += `<div onclick="openChatRoom('${p.id}', '${p.nickname.replace(/'/g, "\\'")}', '${ava}')" class="flex items-center p-3 hover:bg-white/5 cursor-pointer rounded-2xl transition-all"><img src="${ava}" class="w-10 h-10 rounded-full object-cover border border-white/10 shrink-0"><div class="ml-3"><h4 class="font-bold text-white text-xs">${p.nickname}</h4><p class="text-[10px] text-gray-400 truncate w-48">${p.bio || 'Pemain AU2'}</p></div></div>`;
        });
        container.innerHTML = html;
    } else if(container) { container.innerHTML = '<p class="text-center text-xs text-gray-500 mt-6">User tidak ditemukan.</p>'; }
}

function toggleWidget() {
    const widget = document.getElementById('floating-widget');
    if (widget.classList.contains('opacity-0')) {
        history.pushState({ popup: 'widget' }, null, '#inbox'); widget.classList.remove('opacity-0', 'pointer-events-none', 'translate-y-8', 'scale-95'); 
        const searchInput = document.getElementById('search-user-chat'); if(searchInput) searchInput.value = ''; loadChatList();
    } else { 
...
function switchChatTab(tabName) {
    ...
    tabs.forEach(t => {
        const btn = document.getElementById(`tab-${t}`); const container = document.getElementById(`chat-${t}-container`);
        if(!btn || !container) return;
        if (t === tabName) { btn.classList.replace('text-gray-500', 'text-brand-accent'); btn.classList.replace('border-transparent', 'border-brand-accent'); container.classList.remove('hidden'); container.classList.add('block'); } 
        else { btn.classList.replace('text-brand-accent', 'text-gray-500'); btn.classList.replace('border-brand-accent', 'border-transparent'); container.classList.remove('block'); container.classList.add('hidden'); }
    });
...
async function loadChatList() {
    const cPersonal = document.getElementById('chat-personal-container'); const cGroup = document.getElementById('chat-group-container');
    ...
    try {
        const unreadCounts = {}; const { data: unreads } = await supabaseClient.from('messages').select('sender_id').is('group_id', null).eq('receiver_id', currentUser.id).eq('is_read', false);
        ...
        if (messages) {
            messages.forEach(msg => {
                if (msg.group_id) { if (!groupContacts[msg.group_id]) groupContacts[msg.group_id] = msg; } 
                else { const partnerId = msg.sender_id === currentUser.id ? msg.receiver_id : msg.sender_id; if (partnerId && !personalContacts[partnerId] && !blockedUsersList.includes(partnerId)) { personalContacts[partnerId] = msg; } }
            });
        }
        ...
        const renderRow = (item, isGroup) => {
            const ava = item.avatar || `https://ui-avatars.com/api/?name=${item.name}&background=1A1133&color=fff`; const msg = item.latestMsg; let prefix = '', preview = msg.message;
            ...
            return `<div class="flex items-center p-3 bg-brand-card/50 hover:bg-white/5 cursor-pointer rounded-2xl border border-white/5 transition-all mb-2" onclick="openChatRoom('${item.id}', '${item.name.replace(/'/g, "\\'")}', '${ava}', ${isGroup})"><div class="relative shrink-0" onclick="${isGroup ? '' : avatarClickCode}"><div class="${isGroup ? 'border border-brand-accent/30 rounded-full' : ringClass}"><img src="${ava}" loading="lazy" class="w-11 h-11 rounded-full object-cover border-2 border-brand-card"></div>${icon}</div><div class="flex-1 min-w-0 ml-3"><div class="flex justify-between items-center mb-1"><h4 class="font-bold text-white text-[13px] truncate">${item.name}</h4><div class="flex items-center"><span class="text-[9px] text-gray-500 shrink-0 ml-2">${msg.is_empty ? 'Baru' : timeAgo(msg.created_at)}</span>${badge}</div></div><p class="text-[11px] text-gray-400 truncate">${prefix} ${preview}</p></div></div>`;
        };
...
async function kirimPesanPribadi(userId) {
    if (blockedUsersList.includes(userId)) { showToast("Buka blokir terlebih dahulu untuk mengirim pesan.", "error"); return; }
    ...
    if (data) {
        ...
        const widget = document.getElementById('floating-widget');
        if (widget.classList.contains('opacity-0')) { toggleWidget(); }
...
window.openChatRoom = async function(targetId, name, avatar, isGroup = false) {
    ...
    if (isGroup) { activeGroupId = targetId; activeChatUserId = null; } else { activeChatUserId = targetId; activeGroupId = null; }
    ...
    document.getElementById('active-chat-name').innerText = name; document.getElementById('active-chat-avatar').src = avatar;
    ...
    if (localChats.length > 0) { container.innerHTML = ''; localChats.forEach(msg => appendMessageBubble(msg)); scrollToBottomChat(); } 
    else { container.innerHTML = '<div class="flex justify-center mt-10"><i class="fas fa-spinner fa-spin text-brand-accent text-xl"></i></div>'; }
    ...
    if(messages && messages.length > 0) { localStorage.setItem(cacheKey, JSON.stringify(messages)); container.innerHTML = ''; messages.forEach(msg => appendMessageBubble(msg)); } 
    else if (localChats.length === 0) { container.innerHTML = `<div class="flex flex-col items-center justify-center h-full opacity-50"><p class="text-xs text-gray-300">Kirim pesan pertama</p></div>`; }
    scrollToBottomChat();
    ...
    messageSubscription = supabaseClient.channel(isGroup ? `group_${targetId}` : `room_${targetId}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
            const newMsg = payload.new; const isTarget = isGroup ? newMsg.group_id === activeGroupId : (newMsg.sender_id === activeChatUserId && newMsg.receiver_id === currentUser.id);
            if (isTarget) { 
                appendMessageBubble(newMsg); scrollToBottomChat(); if(!isGroup) supabaseClient.from('messages').update({ is_read: true }).eq('id', newMsg.id).then(); 
                let currentCache = JSON.parse(localStorage.getItem(cacheKey) || '[]'); currentCache.push(newMsg); if(currentCache.length > 50) currentCache.shift(); localStorage.setItem(cacheKey, JSON.stringify(currentCache));
            }
        }).subscribe();
};
...
async function sendRoomMessage(targetId, isGroup) {
    ...
    const input = document.getElementById('chat-room-input'); const msg = input.value.trim(); if (!msg) return;
    ...
    try {
        ...
        const { error } = await supabaseClient.from('messages').insert(insertData); if (error) throw error;
    } catch (err) { showToast("Gagal mengirim pesan", "error"); } finally { btn.disabled = false; input.focus(); }
}

function appendMessageBubble(msg) {
    const container = document.getElementById('chat-messages-container'); const isMe = msg.sender_id === currentUser.id;
    ...
    let deletedForMe = JSON.parse(localStorage.getItem('deleted_msgs') || '[]'); if(deletedForMe.includes(msg.id)) return; 
    ...
    else if (msg.message.startsWith('[IMG]')) { const imgUrl = msg.message.substring(5); contentHtml = `<img src="${imgUrl}" alt="Sent Image" class="rounded-lg max-w-[200px] w-full object-cover mb-1 cursor-pointer" onclick="event.stopPropagation(); openLightbox('${imgUrl}')">`; } 
    ...
    div.innerHTML = `<div class="max-w-[80%] px-4 py-2.5 shadow-md relative ${bubbleClass} cursor-pointer active:opacity-80 transition-opacity" ${onclickCode}><div id="bubble-msg-${msg.id}">${contentHtml}</div><div class="flex items-center justify-end gap-1 mt-1"><span class="text-[9px] text-white/60">${timeStr}</span>${tickIcon}</div></div>`;
    container.appendChild(div);
}

function showMsgOptions(msgId, isMe) {
    selectedMessageId = msgId; const modal = document.getElementById('modal-msg-option'); const btnAll = document.getElementById('btn-del-msg-all');
...
async function actionDeleteMessage(type) {
    document.getElementById('modal-msg-option').classList.replace('flex', 'hidden'); if(!selectedMessageId) return;
    if (type === 'me') { let deletedForMe = JSON.parse(localStorage.getItem('deleted_msgs') || '[]'); deletedForMe.push(selectedMessageId); localStorage.setItem('deleted_msgs', JSON.stringify(deletedForMe)); showToast("Pesan dihapus untuk Anda.", "success"); } 
    else if (type === 'all') { const { error } = await supabaseClient.from('messages').update({ message: '[DELETED]' }).eq('id', selectedMessageId); if (error) return showToast("Gagal menghapus pesan.", "error"); }
    if(activeChatUserId || activeGroupId) { const target = activeGroupId ? activeGroupId : activeChatUserId; openChatRoom(target, document.getElementById('active-chat-name').innerText, document.getElementById('active-chat-avatar').src, !!activeGroupId); }
}

function scrollToBottomChat() { const container = document.getElementById('chat-messages-container'); if(container) container.scrollTop = container.scrollHeight; }
...
async function uploadChatImage(event) {
    ...
    try {
        const resUrl = await fetch(`/api/upload-url?filename=${encodeURIComponent('img_'+Date.now())}&filetype=${encodeURIComponent(file.type)}`); const dataUrl = await resUrl.json();
        await fetch(dataUrl.uploadUrl, { method: 'PUT', body: file, headers: { 'Content-Type': file.type, 'x-amz-acl': 'public-read' } });
    ...
    } catch (err) { showToast("Gagal gambar", "error"); }
}

function sendTypingStatus() { if(!messageSubscription || !activeChatUserId) return; messageSubscription.send({ type: 'broadcast', event: 'typing', payload: { userId: currentUser.id } }); }
...
function previewGroupAvatar(input, imgId) {
    if (input.files && input.files[0]) { const reader = new FileReader(); reader.onload = function(e) { document.getElementById(imgId).src = e.target.result; }; reader.readAsDataURL(input.files[0]); }
}
...
async function prosesCreateGroup() {
    ...
    try {
        ...
        if (fileInput.files && fileInput.files[0]) {
            ...
            const resUrl = await fetch(`/api/upload-url?filename=${encodeURIComponent('group_'+Date.now())}&filetype=${encodeURIComponent(file.type)}`); const dataUrl = await resUrl.json();
            await fetch(dataUrl.uploadUrl, { method: 'PUT', body: file, headers: { 'Content-Type': file.type, 'x-amz-acl': 'public-read' } });
        ...
        document.getElementById('create-group-name').value = ''; document.getElementById('create-group-desc').value = ''; fileInput.value = ''; document.getElementById('create-group-preview').src = 'https://ui-avatars.com/api/?name=Grup&background=1A1133&color=fff';
        loadChatList(); 
    } catch (err) { showToast("Gagal buat grup: " + err.message, "error"); } finally { btn.disabled = false; btn.innerHTML = originalText; }
}

function openChatHeaderInfo() { if (activeGroupId) loadGroupInfo(activeGroupId); else if (activeChatUserId) viewUserProfile(activeChatUserId); }
...
async function loadGroupInfo(groupId) {
    ...
    document.getElementById('info-group-avatar').src = group.avatar_url; document.getElementById('info-group-name').innerText = group.name; document.getElementById('info-group-desc').innerText = group.description || "Tidak ada deskripsi";
    ...
    if (activeGroupRole === 'admin') { document.getElementById('btn-edit-group-info').classList.remove('hidden'); document.getElementById('section-invite-group').classList.remove('hidden'); document.getElementById('btn-edit-group-avatar').classList.replace('hidden', 'flex'); } 
    else { document.getElementById('btn-edit-group-info').classList.add('hidden'); document.getElementById('section-invite-group').classList.add('hidden'); document.getElementById('btn-edit-group-avatar').classList.replace('flex', 'hidden'); }
...
async function renderGroupMembers(membersData) {
    ...
    const userIds = membersData.map(m => m.user_id); const { data: profiles } = await supabaseClient.from('profiles').select('id, nickname, avatar_url').in('id', userIds);

    let html = '';
    profiles.forEach(p => {
        const m = membersData.find(x => x.user_id === p.id); const badge = m.role === 'admin' ? '<span class="bg-brand-accent/20 text-brand-accent text-[8px] px-2 py-0.5 rounded border border-brand-accent/30 font-bold ml-2">ADMIN</span>' : '';
        const ava = p.avatar_url || `https://ui-avatars.com/api/?name=${p.nickname}&background=1A1133&color=fff`;
        html += `<div class="flex items-center p-2 bg-black/30 rounded-xl border border-white/5"><img src="${ava}" class="w-8 h-8 rounded-full object-cover border border-white/10 shrink-0"><div class="ml-3 flex-1 flex items-center"><h4 class="font-bold text-white text-xs">${p.nickname}</h4>${badge}</div></div>`;
    });
    list.innerHTML = html;
}
...
function toggleEditGroupInfo() {
    ...
    if (form.classList.contains('hidden')) { form.classList.remove('hidden'); document.getElementById('input-edit-group-name').value = document.getElementById('info-group-name').innerText; document.getElementById('input-edit-group-desc').value = document.getElementById('info-group-desc').innerText; } 
...
async function saveGroupInfo() {
    ...
    btn.innerHTML = 'Simpan Perubahan'; if(error) return showToast("Gagal update: " + error.message, "error");
...
async function keluarDariGrup() { const yakin = confirm("Yakin ingin keluar dari grup ini?"); if(!yakin) return; await supabaseClient.from('group_members').delete().eq('group_id', activeGroupId).eq('user_id', currentUser.id); showToast("Berhasil keluar dari grup", "success"); closeGroupInfoModal(); closeChatRoom(); }

// ==========================================
// 13. RIPPER, INFO, DAN LAYANAN
// ==========================================
async function muatDataRipper() {
    const linkAPI = "/api/ripper"; const container = document.getElementById('ripper-container'); const dataTersimpan = localStorage.getItem('ripperCache');
    if (dataTersimpan) { dataRipperGlobal = JSON.parse(dataTersimpan); renderRippers(dataRipperGlobal, false); } else if (container) { container.innerHTML = '<div class="text-center py-10 text-xs text-brand-info animate-pulse"><i class="fas fa-sync fa-spin mb-2 text-2xl"></i><br>Menyiapkan Database Pertama Kali...</div>'; }
    try { const respon = await fetch(linkAPI); const dataBaru = await respon.json(); localStorage.setItem('ripperCache', JSON.stringify(dataBaru)); dataRipperGlobal = dataBaru; renderRippers(dataRipperGlobal, false); } catch (error) { if (!dataTersimpan && container) { container.innerHTML = '<div class="text-center py-10 text-xs text-red-500"><i class="fas fa-exclamation-triangle mb-2 text-2xl"></i><br>Gagal terhubung ke database.</div>'; } }
}
...
function renderRippers(data, isSearch = false) {
    ...
    const limitBatas = (!isSearch && !isRipperExpanded) ? 5 : data.length; const dataYangDitampilkan = data.slice(0, limitBatas);
...
function tampilkanSemuaRipper(tombol) {
    isRipperExpanded = true; localStorage.setItem('statusLihatSemua', 'true'); tombol.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Membuka Data...'; tombol.classList.add('opacity-70', 'scale-95');
    setTimeout(() => {
...
if(inputRipper) {
    inputRipper.addEventListener('input', (e) => {
        const val = e.target.value.toLowerCase(); if(val === '') { renderRippers(dataRipperGlobal, false); return; }
...
if(inputLayanan){ inputLayanan.addEventListener('input', (e) => { const val = e.target.value.toLowerCase(); renderServices(services.filter(s => s.name.toLowerCase().includes(val) || s.desc.toLowerCase().includes(val))); }); }

function checkout(name, price) {
    const hargaRupiah = price.toLocaleString('id-ID'); document.getElementById('pay-total').innerText = `Rp ${hargaRupiah}`; document.getElementById('pay-product').innerText = name; document.getElementById('nominal-asli').value = price;
    const noWA = "6283815584661"; const teksWA = encodeURIComponent(`Halo Admin Joki AU2Hub,\n\nSaya ingin mengkonfirmasi pembayaran pesanan saya:\n\nPaket : *${name}*\nTotal : *Rp ${hargaRupiah}*\n\n(Berikut ini saya lampirkan gambar bukti transfer QRIS / screenshot)`);
    document.getElementById('wa-confirm').setAttribute('href', `https://wa.me/${noWA}?text=${teksWA}`); switchTab('pembayaran');
}
...
function salinNominal() {
    ...
    if (navigator.clipboard && window.isSecureContext) { navigator.clipboard.writeText(nominalMurni); } else { let tempInput = document.createElement("textarea"); tempInput.value = nominalMurni; tempInput.style.position = "fixed"; tempInput.style.left = "-9999px"; document.body.appendChild(tempInput); tempInput.select(); try { document.execCommand("copy"); } catch (err) {} document.body.removeChild(tempInput); }
...
if(inputFaq){ inputFaq.addEventListener('input', (e) => { const val = e.target.value.toLowerCase(); renderFaqs(faqs.filter(f => f.t.toLowerCase().includes(val) || f.j.toLowerCase().includes(val))); }); }

function openModalEvent() { history.pushState({ popup: 'modal' }, null, '#event'); const modal = document.getElementById('modal-event'); modal.classList.remove('hidden'); modal.classList.add('flex'); document.body.style.overflow = 'hidden'; setTimeout(() => { const carousel = document.getElementById('image-carousel'); if(carousel) { carousel.scrollLeft = 0; updateCarouselDots(); } }, 50); }
function closeModalEvent() { if (window.location.hash === '#event') { history.back(); } else { const modal = document.getElementById('modal-event'); modal.classList.add('hidden'); modal.classList.remove('flex'); document.body.style.overflow = 'auto'; } }
